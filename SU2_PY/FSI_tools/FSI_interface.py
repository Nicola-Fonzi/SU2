...
class Interface:
    ...
    def __init__(self, FSI_config, FluidSolver, SolidSolver, have_MPI):
        ...
        if FSI_config['MATCHING_MESH'] == 'YES':
          ...
          if FSI_config['MESH_INTERP_METHOD'] == 'TPS':
            ...
            self.approximateRBF = FSI_config['APPROXIMATE_RBF'] == 'YES'
    ...
    def interfaceMapping(self, FluidSolver, SolidSolver, FSI_config):
        ...
        for iVertex in range(self.nLocalFluidInterfaceNodes):
            ...
            GlobalIndex = FluidSolver.GetNodeGlobalIndex(iPoint)
            if self.nDim == 2:
                posx, posy = FluidSolver.MarkerInitialCoordinates(self.fluidInterfaceIdentifier).Get(iVertex)
                posz = 0
            else:
                posx, posy, posz = FluidSolver.MarkerInitialCoordinates(self.fluidInterfaceIdentifier).Get(iVertex)
    ...
    def getFluidInterfaceNodalForce(self, FSI_config, FluidSolver):
        ...
        for iVertex in range(self.nLocalFluidInterfaceNodes):
            ...
            if GlobalIndex not in self.FluidHaloNodeList[myid].keys():
              load = FluidSolver.GetMarkerFlowLoad(self.fluidInterfaceIdentifier, iVertex)
              ...
              if self.nDim == 2:
                self.fluidLoads_array_Z.setValues([iGlobalVertex], 0.0)
              else:
                self.fluidLoads_array_Z.setValues([iGlobalVertex], load[2])
              localIndex += 1
    ...
    def setFluidInterfaceVarCoord(self, FluidSolver):
        ...
        for iVertex in range(self.nLocalFluidInterfaceNodes):
            ...
            if GlobalIndex in self.FluidHaloNodeList[myid].keys():
              ...
              if self.nDim == 2:
                FluidSolver.SetMarkerCustomDisplacement(self.fluidInterfaceIdentifier, iVertex, (DispX, DispY))
              else:
                FluidSolver.SetMarkerCustomDisplacement(self.fluidInterfaceIdentifier, iVertex, (DispX, DispY, DispZ))
            else:
    ...
    def MapModes(self, FSI_config, FluidSolver, SolidSolver):
      ...
      if myid == self.rootProcess:  # The root process contains the solid solver for sure
        ...
        modesNumber = np.empty(1, dtype=int)
